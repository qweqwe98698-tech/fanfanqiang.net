...
const targetFile = path.join(__dirname, 'build_runaway_article.js');
...
// Extract everything before \`const newArticleContent = \`\` and after \`\`;\`
const parts = content.split('const newArticleContent = `');
if (parts.length === 2) {
    const endPart = parts[1].substring(parts[1].indexOf('`;'));
    const finalContent = parts[0] + 'const newArticleContent = `' + newContent + endPart;
    fs.writeFileSync(targetFile, finalContent);
    console.log('Successfully updated build_runaway_article.js');
} else {
    console.log('Error splitting content');
}
